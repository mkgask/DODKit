...
再インストール時は、選択したターゲットのマニフェストに宣言されたファイルを管理対象として扱い、同一内容のファイルは変更しません。`--overwrite` を省略した場合、対話端末では `Overwrite this file? [Y/n/a] (a = all remaining files):` と確認し、Enter は現在のファイルを許可し、`n` は現在のファイルを保持し、`a` は現在のファイルを許可したうえで、以降の変更済み管理対象を追加確認なしで上書きします。`--overwrite yes` は確認なしで変更済み管理対象を上書きし、`--overwrite no` は確認なしで保持します。既定の ask ポリシーで利用可能な端末がない場合は、変更済み管理対象を自動更新します。agent と skill files も管理対象のため、それらへのローカル編集は現在のテンプレートで置き換えられる可能性があります。`DECISIONS.yml` だけは例外で、プロジェクトデータとして扱い、`--overwrite yes` や `a` の指定時も上書きしません。
