...
再インストール時は、選択したターゲットのマニフェストに宣言されたファイルを管理対象として扱い、同一内容のファイルは変更しません。`--overwrite` を省略した場合、対話端末では `Overwrite this file? [y/n/A] (A = all remaining files):` と確認し、Enter、`A`、`a` は現在のファイルを許可したうえで、以降の変更済み管理対象を追加確認なしで上書きします。`y` は現在のファイルだけを許可し、`n` は現在のファイルを保持します。`--overwrite yes` は確認なしで変更済み管理対象を上書きし、`--overwrite no` は確認なしで保持します。既定の ask ポリシーで利用可能な端末がない場合は、変更済み管理対象を自動更新します。agent と skill files も管理対象のため、それらへのローカル編集は現在のテンプレートで置き換えられる可能性があります。`DECISIONS.yml` だけは例外で、プロジェクトデータとして扱い、`--overwrite yes` や `A` の指定時も上書きしません。
...
DOD は最終的に責任を取るのは人間という発想に基づく人間主導の手法です。人間が細かな決定を下し、AI は共有された決定事項に基づく調査、実装、検証を支援します。

- **強み:** 現在の拘束条件とその理由を分離し、長期開発の継続性を高め、人間と AI が同じ実装基準を共有できます。
- **弱み:** 意識的な決定・履歴管理が必要で、軽微な変更には重くなりやすく、最終的な品質は人間の判断力に依存します。
- **得意:** 継続的なプロダクト開発、変化する API やデータモデル、複数成果物にまたがる変更、文脈の引き継ぎが必要な協同開発。
- **不得意:** 使い捨ての実験、一回限りのスクリプト、探索性の高い作業、即時性が必要なインシデントの一次対応。
- **適用推奨:** 長期運用する SaaS・業務アプリ、SDK・ライブラリ、OSS、インフラ・CI/CD ツール、AI 支援を継続利用するリポジトリ。
- **総合評価:** 人間による多数の小さな決定を一貫させる継続開発に強く、短期かつ低リスクな作業には意図的に不向きです。
