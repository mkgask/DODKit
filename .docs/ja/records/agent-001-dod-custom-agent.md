...
## 実装更新（2026-04-16）
- ゲートAを明確化し、議論フェーズにおける公式成果物の順序を固定しました。まず `records/{discussion-id}.md` に議論履歴を書き、その後に拘束力のある決定と契約を `DECISIONS.yml` に書き、実装開始はその後に限定します。
- 議論内部での決定候補の反復的な検討は許可しつつ、成果物ゲート順序は固定のまま維持することを明示しました。
