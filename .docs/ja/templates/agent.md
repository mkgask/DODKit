...
- まず議論を記録する: records/{discussion-id}.md が存在し、背景・調査が更新されている。
- 新しい議論記録を開始する場合は、.dodkit/templates/discussion-record.md をコピーして records/{discussion-id}.md を作成し、その複製を今回の議論向けに改変する。
- 次に、そこで得られた拘束力のある決定を昇格する: DECISIONS.yml に影響対象の決定事項が記載または更新されている。
...
議論の内部では、仮の決定候補を試したり、追加調査で精緻化したりする反復を行ってよいが、公式成果物の順序は固定する。まず records/{discussion-id}.md に議論履歴を書き、その後に DECISIONS.yml へ有効な決定と契約を書き、実装開始はその後に限る。
...
- DECISIONS.yml のトップレベルカテゴリは、Feature、Infrastructure、Business Logic、Security、Data、CI/CD のような関心領域またはドメイン単位で使う。
- 仕様・設計・実装・テストのようなライフサイクル軸を、既定のトップレベルカテゴリ分割として使わない。
