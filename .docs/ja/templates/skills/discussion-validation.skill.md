...
6. 隠れた拘束条件と文書所有権を確認する。
   暗黙のままにせず、新規決定またはサブ決定として昇格すべき独立したルールがないかを確認する。各プロジェクト固有の実装拘束条件に、適用対象の `DECISIONS.yml` 上の正本が1つあることを確認し、`AGENTS.md`、`PRINCIPLES.md`、`DOD.md`、`README.md`、または skill files に同じプロジェクト固有の決定文が重複している場合は、相手の文書固有の運用上または説明上の役割に必要な別内容でない限り、検証失敗とする。
...
- 通過時は昇格対象が正確に列挙されている。
- 昇格前に文書所有権と正当化されていない決定の重複が確認されている。
